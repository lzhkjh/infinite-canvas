        ...
        args={[
          {
            ...
            fragmentShader: `
              ...
              void main() {
                vec2 uv = vUv;
                vec2 halfCorner = vec2(uRadius);
                vec2 dist = abs(uv - 0.5);
                vec2 cornerDist = max(dist - (0.5 - halfCorner), 0.0);
                float corner = 1.0 - smoothstep(0.0, uRadius, length(cornerDist));
                float inside = step(max(dist.x, dist.y), (0.5 - uRadius));
                float alpha = max(inside, corner);
...
function SceneController({ media, onTextureProgress, gestureState }: { media: MediaItem[]; onTextureProgress?: (progress: number) => void; gestureState?: any; onPlaneClick?: (item: MediaItem | null) => void }) {
  ...
  useFrame(() => {
    ...
    if (backward) s.targetVel.z += KEYBOARD_SPEED;
      if (gesture.backward) s.targetVel.z += KEYBOARD_SPEED * gesture.velocity;
    ...
    if (right) s.targetVel.x += KEYBOARD_SPEED;
      if (gesture.right) s.targetVel.x += KEYBOARD_SPEED * gesture.velocity;
...
if (gesture.up) s.targetVel.y += KEYBOARD_SPEED * gesture.velocity;
    ...
    <KeyboardControls map={KEYBOARD_MAP}>
      <div className={styles.container}>
        ...
        >
          ...
          <SceneController media={media} onTextureProgress={onTextureProgress} gestureState={gestureState} onPlaneClick={handlePlaneClick} />
        ...
        {showControls && (
          <div className={styles.controlsPanel}>
            {isTouchDevice ? (
              <>
                <b>Drag</b> Pan é—?<b>Pinch</b> Zoom
              </>
            ) : (
              <>
                <b>WASD</b> Move é—?<b>QE</b> Up/Down é—?<b>Scroll/Space</b> Zoom
              </>
            )}
          </div>
        )}
